...
import {
  ...
  ThemeExtractionResponse as ThemeExtractionResponseSchema,
} from "@/lib/schemas";
...
export async function POST(request: Request) {
  try {
    ...
    const submission = SetupSubmissionSchema.safeParse(body);
    if (!submission.success) {
      return NextResponse.json(
        { error: "Invalid request body", details: submission.error.message },
        { status: 400 },
      );
    }

    const { creatorProfile, commentInput } = submission.data;
    ...
    const prompt = buildThemeExtractionPrompt({
      ...
      postTopic: commentInput.postTopic,
      postCaption: commentInput.postCaption,
    ...
    const validated = ThemeExtractionResponseSchema.safeParse(parsed);
    ...
    console.error("analyze-themes error:", message);
